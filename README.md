...
Today `modules/data/world.bin` and `modules/data/world-pool-mountain-v3.bin` are the same file: a 20 m × 20 m world with a mountain, a pool of static water, and one diffuse-daylight cell. `modules/data/world-v3-ramp.bin` remains the 10 m × 10 m ramp. `modules/data/world-v3-mound.bin` is another version 3 world, a mound 2 m above a flat border, generated by [modules/utils/create_world_v3_mound.py](modules/utils/create_world_v3_mound.py).
